...
- 单个节点允许同时具备多个角色，例如稳定控制节点也可兼任服务提供方。
- 角色只用于表达职责来源，不直接等同于最终权限。

### 2.3 三组互斥能力清单

为避免多角色节点出现职责混淆，运行时权限应收敛到三组互斥能力中：
...
- 服务发布能力
  - `none`
  - `publish_owned_services`
- 服务消费能力
  - `none`
  - `resolve_only`
  - `resolve_select_watch_and_report`

约束原则：

- 一个节点可以有多个角色，但每组能力在任一时刻只能取一个最终值。
- 最终能力由角色集合映射得出，而不是简单叠加布尔开关。
- `A + B` 同机部署时，允许同时具备“管理目录”和“发布本节点服务”，但不应出现两套相互独立的发布职责。
- 这一模型可参考 Kubernetes 的 control-plane / worker、以及边缘集群中“稳定控制节点兼任工作负载”的做法。
