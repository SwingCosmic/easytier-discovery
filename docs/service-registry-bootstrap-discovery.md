...
完整应用层 API 表见 [应用接入 §4](./service-registry-application-layer.md#4-api-面)。
...
实例 CRUD 见 [应用接入 §4](./service-registry-application-layer.md#4-api-面)。
...
- Core：角色 bit 约定、纯逻辑
- Runtime 宿主：EasyTier bridge、HTTP API、role host
- 业务 SDK：调用控制面完成注册/选点；VIP/nodeId 由 SDK 或轻量观测填入；**不**经本地 `/runtime/v1` 代理写注册表
